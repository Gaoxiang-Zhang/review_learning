...
def _RunningAvgLoss(loss, running_avg_loss, summary_writer, step, decay=0.999):
  ...
  # 在decay系数的作用下计算average loss（和see的函数相同）
...
def _Train(model, data_batcher):
  """Runs model training."""
  # 输入是Seq2SeqAttentionModel对象和batcher对象
  with tf.device('/cpu:0'):
    # 构建图，初始化saver，summary_writer，supervisor和sess
    model.build_graph()
