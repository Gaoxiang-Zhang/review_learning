...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  """
  Args:
    decoder_inputs: A list of 2D Tensors [batch_size x embed_size]. （是decoder输入的embedding，长度为max_step，每个元素为batch_size * embed_size）
    initial_state: 2D Tensor [batch_size x cell.state_size]. (reduce states的结果，一个包含c和h的tuple)
    encoder_states: 3D Tensor [batch_size x attn_length x attn_size]. （encoder的输出，[batch_size, <=max_enc_steps, 2*hidden_dim]）
    enc_padding_mask: 2D Tensor [batch_size x attn_length] containing 1s and 0s; indicates which of the encoder locations are padding (0) or a real token (1). （encoder的padding mask）
    cell: rnn_cell.RNNCell defining the cell function and size. （decoder的lstm cell）
    initial_state_attention: （如果是false，第一步的context vec是0，用于train；否则用initial_state计算，用于decode）
    ...
    prev_coverage:
      If not None, a tensor with shape (batch_size, attn_length). The previous step's coverage vector. This is only not None in decode mode when using coverage.（之前的coverage，只在decode时使用）
  Returns:
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    # 得到batch_size和attn_size(其中attn size也就是2*hidden)
    ...
    # 把encoder states变成4维的形式（后面会做conv2d）
    encoder_states = tf.expand_dims(encoder_states, axis=2) # now is shape (batch_size, attn_len/max_steps, 1, attn_size/2hidden)
    ...
    # 每一项的长度都是attention_vec_size
    attention_vec_size = attn_size

    # Get the weight matrix W_h and apply it to each encoder state to get (W_h h_i), the encoder features
    W_h = variable_scope.get_variable("W_h", [1, 1, attn_size, attention_vec_size])
    # input, filter, strides, padding
    # input的格式(batch, in_height, in_width, in_channels)
    # filter的格式(filter_height, filter_width, in_channels, out_channels), in_channels和input的一样
    # 所以这里输出的维度是(batch_size,attn_length,1,attention_vec_size)，应该是Wh*hi
    ...
    def attention(decoder_state, coverage=None):
      ...
      # 根据decoder state计算attention
      Args:
        decoder_state: state of the decoder （decoder的states）
        coverage: Optional. Previous timestep's coverage vector, shape (batch_size, attn_len, 1, 1). （之前的coverage vector）
      ...
      with variable_scope.variable_scope("Attention"):
        ...
        # decoder features，论文中的W_s s_t,linear函数可以看做是w*s
        ...
        if use_coverage and coverage is not None: # non-first step of coverage
          ...
          if use_coverage: # first step of training
            # 如果是第一次算coverage，则coverage为当前的计算结果（进行扩展）
            coverage = tf.expand_dims(tf.expand_dims(attn_dist,2),2) # initialize coverage

        # Calculate the context vector from attn_dist and encoder_states
        # 计算context vector，按照公式（3）
    ...
    attn_dists = []
    p_gens = []
    # 初始是encoder的final state（包含c和h）
    state = initial_state
    coverage = prev_coverage # initialize coverage to None or whatever was passed in

    # batch_size * 2hidden_dim，论文中的公式（3）
    ...
    # 每个step的tensor是batch_size * embed_size
